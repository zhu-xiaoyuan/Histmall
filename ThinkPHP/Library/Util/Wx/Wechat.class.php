...
class Wechat
{
    ...
    protected function log()
    {
        if ($this->debug && function_exists($this->logcallback)) {
            ...
            return call_user_func($this->logcallback, func_get_args());
        }
    }
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    protected function setCache($cachename, $value, $expired)
    {
        ...
        if ($cachename == 'wechat_jsapi_ticket' . $this->appid) {
            S('wechat_jsapi_ticket', $value, $expired);
        }
        return false;
    }
    ...
    protected function removeCache($cachename)
    {
        ...
        if ($cachename == 'wechat_jsapi_ticket' . $this->appid) {
            return S('wechat_jsapi_ticket', null);
        }
        return false;
    }
    ...
    public function getOauthUserinfo($access_token, $openid)
    {
        ...
        $this->log(['OAUTHUSERINFO' => 'getOauthUserinfo'], $result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function setTMIndustry($id1, $id2 = '')
    {
        ...
        $this->log(['TMIndustry' => '设置所属行业'], $result);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
